...
'MsgBox "DEBUG MODE" & vbNewLine & Me.BorderStyle
...
Private Sub Form_MouseUp(Button As Integer, Shift As Integer, x As Single, Y As Single)
    ...
    If Me.MousePointer = 2 Then
        ...
        Case .Buttons(4).Value
            ...
            Case .Buttons(5).Value

            If frameTemp(0).Tag = "" Then frameTemp(0).Tag = 0
            frameTemp(0).Tag = frameTemp(0).Tag + 1
            Load frameTemp(frameTemp(0).Tag)
            'object(object(0).Tag).Move StartX, StartY, EndX - StartX, EndY - StartY
        frameTemp(frameTemp(0).Tag).Move StartX, StartY, EndX - StartX, EndY - StartY
    frameTemp(frameTemp(0).Tag).Visible = True
          frameTemp(frameTemp(0).Tag).Tag = "Frame" & frameTemp(0).Tag
...
Private Sub frameTemp_MouseDown(Index As Integer, Button As Integer, Shift As Integer, x As Single, Y As Single)
    StartX = x
    StartY = Y
    XPos = frameTemp(Index).Left
    YPos = frameTemp(Index).Top
    
    Set selectedControl = frameTemp(Index)
    With selectedControl
        picResize.Move .Left + .width, .Top + .height
        picResize.Visible = True
        '.Tag = Index
...
Private Sub frameTemp_MouseMove(Index As Integer, Button As Integer, Shift As Integer, x As Single, Y As Single)
    Dim yNew As Integer, xNew As Integer
    If Button = 1 Then
        With frameTemp(Index)
            yNew = GridY(YPos - (Pixels(StartY - Y)))
            xNew = GridX(XPos - (Pixels(StartX - x)))
...
Private Sub frameTemp_MouseUp(Index As Integer, Button As Integer, Shift As Integer, x As Single, Y As Single)
    With frameTemp(Index)
        .Move GridX(XPos - (Pixels(StartX - x))), GridY(YPos - (Pixels(StartY - Y)))
        shpMove.Visible = False
    End With
    ...
    FillProperties frameTemp(Index), Frame_props
    FillControlList frameTemp(Index)
End Sub
